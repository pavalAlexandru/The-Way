...
function GameBoard({ token, username, onLogout }) {
  const [sessionId, setSessionId] = useState(null);
  const [column, setColumn] = useState(1);
  const [msg, setMsg] = useState("Generating board...");
  const [endData, setEndData] = useState(null);
  ...
  // Auto-start the game immediately after login (Exam requirement)
  useEffect(() => {
  ...
  const handleCellClick = async (row) => {
    ...
    try {
      ...
      if (data.status === "Active") {
        ...
        setMsg(
          data.status === "Won"
            ? `YOU WON! Final Score: ${data.score}`
            : `GAME OVER! You hit an obstacle or deviated from the safe path.`,
        );
      }
    } catch (err) {
  ...
  // Exam Requirement: Show path and obstacles directly "pe tabla" (on the board)
  const getCellColor = (row, col) => {
    if (!endData) return col === column ? "#3498db" : "#bdc3c7";

    const isSafe = endData.safePath.split(",")[col - 1] === row.toString();
    const isObstacle = endData.obstacles.split(",").includes(`${row}-${col}`);

    if (isSafe) return "#2ecc71"; // Green for safe path
    if (isObstacle) return "#e74c3c"; // Red for obstacles
    return "#bdc3c7"; // Gray for everything else
  };
  ...
  return (
    <div className="game-board-wrapper">
      <div className="game-header">
        <h2>Playing as: {username}</h2>
        {endData && <button onClick={startGame}>Play Again</button>}
        <h3>{msg}</h3>
      </div>
